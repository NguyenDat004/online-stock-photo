    ...
    <div style={{ backgroundColor: '#f8f9fa', minHeight: '100vh', paddingTop: '2rem', paddingBottom: '2rem' }}>
      <Container>
        ...
        <div className="d-flex justify-content-between align-items-center mb-4">
          <div>
            <h2 className="fw-bold mb-2">📦 Chi tiết đơn hàng #{transactionId}</h2>
            <p className="text-muted mb-0">Xem thông tin chi tiết và tải về các ảnh đã mua</p>
          </div>
          <Button 
        ...
        <Card className="shadow-sm border-0">
          <Card.Body>
            <h5 className="fw-bold mb-4">Danh sách ảnh đã mua ({orderItems.length})</h5>
            
            {orderItems.length === 0 ? (
              <div className="text-center py-5">
                <div style={{ fontSize: '3rem' }}>📷</div>
                <p className="text-muted mt-3">Không có ảnh nào trong đơn hàng này</p>
              </div>
            ) : (
              <Row className="g-4">
                {orderItems.map((item) => (
                  <Col lg={4} md={6} key={item.photo_id}>
                    <Card className="h-100 shadow-sm border-0 hover-shadow" style={{ transition: 'transform 0.2s, box-shadow 0.2s' }}>
                      <div style={{ 
                        height: '280px', 
                        overflow: 'hidden',
                        ...
                        <Card.Img 
                          variant="top" 
                          src={item.image_url || 'https://via.placeholder.com/400x280?text=No+Image'} 
                          alt={item.title}
                          style={{ 
                            width: '100%',
                            height: '100%',
                          ...
                          onError={(e) => {
                            e.target.src = 'https://via.placeholder.com/400x280?text=Image+Error';
                          }}
                        />
                      </div>
                      ...
                      <Card.Body className="d-flex flex-column">
                        ...
                        <Card.Text className="text-muted mb-2" style={{ fontSize: '0.9rem' }}>
                          Ngày mua: {formatDate(item.created_at || orderInfo?.created_at)}
                        </Card.Text>
